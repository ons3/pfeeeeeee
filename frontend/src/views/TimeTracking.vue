...
const EMPLOYEE_ID = 'deefb27e-f528-4e66-bd06-906929c00016';
...
const startTracking = async () => {
    ...
    try {
        ...
        const { data } = await createTimeEntry({
            input: {
                heure_debut_suivi: backupData.startTime,
...
onMounted(() => {
    onActiveEntryResult((result) => {
        if (result.data?.getActiveSuivi) {
            ...
            const backup = JSON.parse(localStorage.getItem(LOCAL_STORAGE_KEY));
            if (backup?.startTime) {
                resumeTracking(backup.startTime, backup.taskId, backup.projectId);
            }
        }
    });
});
                    ...
                    <div class="tracker-controls">
                        <div class="control-group">
                            <div class="dropdown-group">
                                ...
                                <Dropdown v-model="selectedProject" :options="projects" optionLabel="name" placeholder="Select Project" :disabled="isRunning" class="dropdown" @change="handleProjectChange" />
                            </div>

                            <div class="dropdown-group">
                                <label class="control-label">Task</label>
                                <Dropdown v-model="selectedTask" :options="filteredTasks" optionLabel="title" placeholder="Select Task" :disabled="!selectedProject || isRunning" class="dropdown" />
                            </div>
                        </div>
